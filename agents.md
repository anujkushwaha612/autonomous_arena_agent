...
- T8 completed: migration v5 adds transaction `reconciled_at`; account reconciliation compares the statement/ledger balance through an as-of date, settles matching pending transactions, and reports mismatches; full-ledger JSON/CSV export with attachment headers and CSV quoting; health count fields; CLI reconcile/export and top-level JSON output; install-free README.
